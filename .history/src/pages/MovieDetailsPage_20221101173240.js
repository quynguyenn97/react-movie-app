...
const MovieDetailsPage = () => {
    ...
    const tvShowDetailsApi = useSWR(tmdbAPI.getTVDetails(movieId), fetcher);
    const movieDetailsApi = useSWR(tmdbAPI.getMovieDetails(movieId), fetcher);
    const { data } = tvShowDetailsApi;
    if (!data) return null;
    ...
    return (
        <div>
            ...
            <div className="w-[200px] relative z-9 pb-10 -mt-[150px] mx-auto md:h-[600px] md:-mt-[400px] md:w-[400px] lg:-mt-[400px]">
                <img
            ...
            <p className="text-center leading-relaxed max-w-[600px] mx-auto mb-5">
                {overview}
            </p>
...
function MovieMeta({ type = "videos" }) {
    const { movieId } = useParams();
    const tvShowMetaApi = useSWR(tmdbAPI.getTVMeta(movieId), fetcher);
    const movieMetaApi = useSWR(tmdbAPI.getMovieMeta(movieId, type), fetcher);
    const { data } = tvShowMetaApi;
    if (!data) return null;
    if (type === "credits") {
        const { cast } = data;
